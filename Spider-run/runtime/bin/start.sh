...
JAVA_MEM_SIZE_OPTS="-Xmx512m -Xms378m -Xmn256m -XX:PermSize=64m -XX:MaxPermSize=256M -Xss256k"
...
while [ $COUNT -lt 1 ]; do    
    ...
    if [ -n "$SERVER_PROTOCOL_PORT" ]; then
        if [ "$SERVER_PROTOCOL_NAME" == "dubbo" ]; then
            COUNT=`(echo "status";sleep 0.2;exit)|telnet 127.0.0.1 29019 2> /dev/null | grep -c OK`
        else
